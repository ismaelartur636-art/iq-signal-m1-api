...
class OlympTradeClient:
    ...
    async def send_request(self, event_code: int, data: Any, requires_response: bool = True, timeout: Optional[float] = None) -> Optional[Dict[str, Any]]:
        ...
        """ Sends a request to the WebSocket server and optionally waits for a response. Args: event_code: The integer event code for the request. data: The data payload for the request (usually a list of dicts). requires_response: If True, waits for a response matched by UUID. timeout: Custom timeout for waiting for the response. Uses default if None. Returns: The parsed response dictionary if requires_response is True, otherwise None. Raises: ConnectionError: If the WebSocket is not connected. asyncio.TimeoutError: If waiting for a response times out. Exception: For other send/serialization errors. """
...
@property
    ...
    async def initialize_session(self):
        """ Sends the required subscription, ping, and account info requests after connecting. This mimics the browser's startup sequence. """
    ...
    async def wait_for_balance(self, timeout: float = 10.0, poll_interval: float = 0.5):
        """ Waits until a balance update is received or timeout is reached. Returns the balance dict if received, else None. """
