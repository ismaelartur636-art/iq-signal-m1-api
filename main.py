...
app = FastAPI(title="MEGA IA", version="32.1.0")
    ...
    if market == "OTC" and iq_state is not None:
        ...
        if iq_state.get("reconnecting") or time.time() < ra or not _iq_is_connected(iq_state):
            neutral = neutral_signal(
                ...
                "A sessão OTC está reconectando. Nenhuma entrada será liberada.",
...
async def radar(request: Request, interval="1min", market="OPEN"):
    ...
    if market == "OTC":
        ...
        if not state:
            return [
                {"symbol": s+" • OTC", "direction":"NEUTRO", "confidence":0, "status":"LOGIN IQ OPTION"}
                for s in SYMBOLS
            ]
        ...
        for sym in SYMBOLS:
            ...
            try:
                ...
                out.append({
                    "symbol":sym+" • OTC",
                    "direction":direction,
                    "confidence":round(tech.get("confidence",0),1),
    ...
    out=list(previous[1]) if previous else [
        {"symbol":sym,"direction":"NEUTRO","confidence":0,"status":"AGUARDANDO LEITURA"}
        for sym in SYMBOLS
    ]
