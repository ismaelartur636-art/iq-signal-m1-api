...
IQ_IMPORT_ERROR = ""
IQ_ASYNC_IMPORT_ERROR = ""
IQ_STABLE_IMPORT_ERROR = ""
...
try:
    ...
    IQ_ASYNC_IMPORT_ERROR = f"{type(exc).__name__}: {exc}"

try:
    from iqoptionapi.stable_api import IQ_Option as StableIQOption
except Exception as exc:
    StableIQOption = None
    IQ_STABLE_IMPORT_ERROR = f"{type(exc).__name__}: {exc}"

IQ_IMPORT_ERROR = " | ".join(x for x in (IQ_ASYNC_IMPORT_ERROR, IQ_STABLE_IMPORT_ERROR) if x)

from fastapi import FastAPI, HTTPException, Request, Response
from pydantic import BaseModel
from fastapi.responses import HTMLResponse, FileResponse, JSONResponse

app = FastAPI(title="MEGA IA", version="32.10.1")
...
class _StableIQAdapter:
    """Adapta a API síncrona estável para o restante assíncrono do MEGA IA."""
    ...
    async def get_candles(self, active, size, count, endtime, timeout=None):
        return await asyncio.to_thread(
            self.api.get_candles, active, int(size), int(count), endtime
        )
    ...
    async def close(self):
        ...
        def _close_sync():
            try:
                ...
                pass
            try:
                low = getattr(api, "api", None)
                close_fn = getattr(low, "close", None) if low is not None else None
                if callable(close_fn):
                    close_fn()
            except Exception:
                pass
...
async def _iq_connect_stable(email: str, password: str):
    if StableIQOption is None:
        raise RuntimeError("Cliente estável da IQ Option não carregado no servidor.")

    _set_iq_login_diag("stable_connect_start", "Iniciando login pela API estável.")

    def _connect_sync():
        api = StableIQOption(email, password)
        result = api.connect()
        ...
        if isinstance(result, (tuple, list)):
            if result:
                ok = bool(result[0])
            if len(result) > 1:
                reason = _iq_reason_text(result[1])
        elif isinstance(result, bool):
            ok = result

        if ok is None:
            checker = getattr(api, "check_connect", None)
            if callable(checker):
                try:
                    ok = bool(checker())
                except Exception:
        ...
        if not ok:
            try:
                ...
                pass
            raise RuntimeError(reason or "A IQ Option recusou a autenticação.")
        return api

    try:
        api = await asyncio.wait_for(asyncio.to_thread(_connect_sync), timeout=35.0)
    except asyncio.TimeoutError as exc:
        _set_iq_login_diag("stable_connect_timeout", "Timeout no login pela API estável.")
        raise RuntimeError("A IQ Option não respondeu ao login dentro de 35s.") from exc

    _set_iq_login_diag("websocket_authenticated", "Login concluído pela API estável da IQ Option.")
    return _StableIQAdapter(api)
...
async def _iq_connect_official(email: str, password: str):
    """Conecta usando a API estável quando disponível; usa o cliente async como fallback."""
    if StableIQOption is not None:
        return await _iq_connect_stable(email, password)

    if AsyncIQOption is None:
        raise RuntimeError(
            "Nenhum cliente IQ Option foi carregado no servidor. "
            + (IQ_IMPORT_ERROR or "Verifique o requirements.txt.")
        )
    ...
    _set_iq_login_diag("async_connect_start", "API estável ausente; usando cliente assíncrono.")
    ...
    try:
        ...
        await asyncio.wait_for(client.connect(), timeout=35.0)
        ...
        _set_iq_login_diag("websocket_authenticated", "Login HTTP e WebSocket concluídos pelo cliente assíncrono.")
        return client

    except asyncio.TimeoutError as exc:
        _set_iq_login_diag("async_connect_timeout", f"Timeout ao acessar {login_url}")
        raise RuntimeError("A IQ Option não respondeu ao login dentro de 35s.") from exc
    except IQTwoFactorRequired:
        raise
    except Exception as exc:
        _set_iq_login_diag("async_connect_failed", f"{type(exc).__name__}: {str(exc)[:180]}")
        raise


async def iq_connect_async(state, force=False):
    if StableIQOption is None and AsyncIQOption is None:
...
async def health():
    ...
    return {
        ...
        "iq_option": {
            "library": bool(StableIQOption is not None or AsyncIQOption is not None),
            "stable_library": bool(StableIQOption is not None),
            "async_library": bool(AsyncIQOption is not None),
...
async def iq_diagnostic():
    info = {
        "app_version": "32.10.1",
        "iq_stable_library_loaded": StableIQOption is not None,
        "iq_async_library_loaded": AsyncIQOption is not None,
        "import_error": IQ_IMPORT_ERROR if (StableIQOption is None and AsyncIQOption is None) else "",
...
async def iq_port_test():
    ...
    result = {"app_version": "32.10.1", "port": 443, "hosts": {}}
...
async def iq_login(body: IQLoginBody, response: Response):
    ...
    print(f"[IQ LOGIN] tentativa para domínio={email.split('@')[-1] if '@' in email else 'invalido'} biblioteca_stable={'OK' if StableIQOption is not None else 'AUSENTE'} biblioteca_async={'OK' if AsyncIQOption is not None else 'AUSENTE'}", flush=True)
...
async def version_info():
    return JSONResponse(
        {"app": "MEGA IA", "version": "32.10.1", "js": "ready", "license": "disabled"},
