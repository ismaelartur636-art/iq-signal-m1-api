...
app = FastAPI(title="MEGA IA", version="14.1.0")
...
def iq_connect_blocking(force=False):
    global iq_client, iq_last_error, iq_last_attempt
    if not IQ_ENABLED:
    ...
    if iq_client is not None:
        try:
            if bool(iq_client.check_connect()):
                return iq_client
        except Exception:
            pass
        iq_client = None

    # Pequeno backoff para evitar centenas de reconexões por minuto.
    if iq_last_error and now_ts - iq_last_attempt < 3:
        raise RuntimeError(iq_last_error)

    iq_last_attempt = now_ts
    client = IQ_Option(IQ_EMAIL, IQ_PASSWORD)
    ok, reason = client.connect()
    if not ok:
        iq_last_error = f"Falha ao conectar na IQ Option: {reason}"
        raise RuntimeError(iq_last_error)

    # A biblioteca pode retornar connect=True antes do websocket ficar pronto.
    ready = False
    for _ in range(10):
        try:
    ...
    if not ready:
        iq_last_error = "A IQ Option aceitou o login, mas o websocket OTC não permaneceu conectado."
        raise RuntimeError(iq_last_error)
...
def iq_candles_blocking(symbol, interval, n):
    global iq_client, iq_last_error
    duration = iq_seconds(interval)
    candidates = iq_active_candidates(symbol)
    errors = []

    # Duas rodadas: a segunda recria totalmente a sessão caso get_candles peça reconnect.
    for round_no in range(2):
        client = iq_connect_blocking(force=(round_no > 0))
        for name in candidates:
            try:
                ...
                # 'need reconnect' indica que esta instância não deve ser reutilizada.
    ...
    iq_last_error = f"OTC conectado no login, mas sem candles. {detail}"
    raise RuntimeError(iq_last_error)
...
async def candles(symbol, interval, n=80, market="OPEN"):
    ...
    if market == "OTC":
        ...
        cached = iq_candle_cache.get(ckey)
        if cached and time.time() - cached[0] < 5:
            return cached[1][-n:]
        try:
            async with iq_lock:
                # Reconfere o cache após esperar pelo lock: gráfico e sinal chegam quase juntos.
                cached = iq_candle_cache.get(ckey)
                if cached and time.time() - cached[0] < 5:
                    return cached[1][-n:]
                out = await asyncio.wait_for(asyncio.to_thread(iq_candles_blocking, symbol, interval, n), timeout=18)
                iq_candle_cache[ckey] = (time.time(), out)
        except Exception as exc:
...
@app.get("/health")
async def health():
    return {"status": "ok", "app": "MEGA IA", "version": "14.1.0", "brasilia_time": iso(now()),
...
@app.get("/otc-status")
async def otc_status():
    if IQ_Option is None:
        return {"configured": False, "connected": False, "message": "Biblioteca IQ Option não foi carregada. Verifique o requirements.txt e faça um novo deploy."}
    if not IQ_EMAIL or not IQ_PASSWORD:
        return {"configured": False, "connected": False, "message": "Preencha IQ_OPTION_EMAIL e IQ_OPTION_PASSWORD no Render."}
    try:
        async with iq_lock:
            client = await asyncio.wait_for(asyncio.to_thread(iq_connect_blocking), timeout=15)
        connected = bool(getattr(client, "check_connect", lambda: False)())
        return {"configured": True, "connected": connected, "message": "IQ Option OTC conectada." if connected else "IQ Option OTC desconectada.", "pairs": len(OTC_BASE)}
    except Exception as exc:
