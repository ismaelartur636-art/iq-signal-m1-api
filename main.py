...
app = FastAPI(title="MEGA IA", version="28.0.0")
...
async def candles(symbol, interval, n=80, market="OPEN", iq_state=None):
    ...
    if market == "OTC":
        ...
        try:
            ...
            # Durante uma reconexão curta, reaproveita o último candle válido
            # apenas para manter o painel/gráfico vivo. O motor de sinal continua
            # usando candle fechado e a resposta é marcada via cache local.
            stale = candle_cache.get(ckey)
            if stale and time.time() - stale[0] < 180:
                return stale[1][-n:]
...
@app.get("/health")
async def health():
    return {"status": "ok", "app": "MEGA IA", "version": "28.0.0", "brasilia_time": iso(now()),
...
async def manifest():
    manifest_data = {
        ...
        "icons": [
            {
                "src": "/mega-ia-icon-192.png?v=28",
                "sizes": "192x192",
                "type": "image/png",
                "purpose": "any"
            },
            {
                "src": "/mega-ia-icon.png?v=28",
                "sizes": "512x512",
                "type": "image/png",
                "purpose": "any"
            },
...
async def radar(request: Request, interval="1min", market="OPEN"):
    ...
    if market == "OTC":
        ...
        if rkey in radar_cache and time.time() - radar_cache[rkey][0] < 60:
            return radar_cache[rkey][1]
        out=[]
        # Se a sessão estiver em reconexão, o radar não dispara 13 chamadas de
        # candles. Isso protege o websocket e reduz drasticamente os erros no log.
        reconnect_after = float(state.get("reconnect_after", 0.0) or 0.0)
        if state.get("reconnecting") or time.time() < reconnect_after or not _iq_is_connected(state):
            cached_radar = radar_cache.get(rkey)
            if cached_radar:
                return cached_radar[1]
            return [{"symbol": sym + " • OTC", "direction": "NEUTRO", "confidence": 0, "status": "IQ OPTION RECONECTANDO"} for sym in SYMBOLS]
