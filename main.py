...
IQ_RECONNECT_MAX_DELAY = float(os.getenv("IQ_RECONNECT_MAX_DELAY", "45"))
...
def _iq_mark_failure(state, message):
    ...
    delay = min(IQ_RECONNECT_MAX_DELAY, IQ_RECONNECT_BASE_DELAY * (2 ** min(failures - 1, 3)))
...
def iq_candles_blocking(state, symbol, interval, n):
    ...
    for name in candidates:
        try:
            ...
            low = msg.lower()
            if any(k in low for k in ("reconnect", "disconnect", "is_ssl", "websocket", "closed")):
