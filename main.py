...
async def otc_status():
    ...
    try:
        async with iq_lock:
            client = await asyncio.to_thread(iq_connect_blocking)
        connected = bool(getattr(client, "check_connect", lambda: False)())
        return {"configured": True, "connected": connected, "message": "IQ Option OTC conectada." if connected else "IQ Option OTC desconectada."}
    except Exception as exc:
