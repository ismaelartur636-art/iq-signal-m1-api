...
app = FastAPI(title="MEGA IA", version="15.0.0")
...
IQ_SESSION_TTL = int(os.getenv("IQ_SESSION_TTL", "14400"))  # 4 horas
...
def _session_state(request: Request, required: bool = False):
    ...
    token = request.cookies.get(IQ_SESSION_COOKIE, "")
...
async def openai_confirm(symbol, interval, cs, analysis):
    ...
    prompt = f'''Você é o módulo de confirmação da MEGA IA. Ativo {symbol}, timeframe {interval}. Use SOMENTE candles fechados. Não invente dados futuros. Estratégias: Bollinger 20/2 + Estocástico 14,3,3 e EMA 9/21 + RSI 7. Análise técnica preliminar: {json.dumps(analysis, ensure_ascii=False)} Retorne SOMENTE JSON: {{"direction":"CALL|PUT|NEUTRO","confidence":0,"confirmed":true,"reason":"curto","risk":"LOW|MEDIUM|HIGH"}} Candles: {json.dumps(data, ensure_ascii=False)}'''
    try:
...
@app.get("/health")
async def health():
    return {"status": "ok", "app": "MEGA IA", "version": "15.0.0", "brasilia_time": iso(now()),
...
async def iq_login(body: IQLoginBody, response: Response):
    ...
    return {"connected": True, "message": "IQ Option OTC conectada.", "email_masked": _mask_email(email)}
...
async def otc_status(request: Request):
    ...
    try:
        ...
        return {
            ...
            "message": "IQ Option OTC conectada." if connected else "IQ Option OTC desconectada.",
            "email_masked": _mask_email(state.get("email", "")),
            "pairs": len(OTC_BASE)
        }
    except Exception as exc:
        return {"configured": True, "connected": False, "message": f"Falha IQ Option OTC: {str(exc)[:300]}"}
...
async def radar(request: Request, interval="1min", market="OPEN"):
    ...
    market=(market or "OPEN").upper()
    if market == "OTC":
        # Não abre 13 requisições simultâneas no websocket da IQ Option.
        # O sinal e o gráfico do par selecionado continuam em tempo real.
        return [{"symbol": s + " • OTC", "direction": "NEUTRO", "confidence": 0, "status": "OTC • selecione o par"} for s in SYMBOLS]
